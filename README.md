...
## Как посчитать динамическую плотность:
### Шаг 1: Детекции
Для начала нужно получить файл с предсказанными ограничивающими рамками и ключевыми точками объектов. Для этого нужно запустить [файл](#universal_RCNN_test.py), передав в него путь видеофайла, для которого нужно получить предсказания, и путь, по которому сохранены веса модели *(актуальные веса ищите в последнем релизе проекта)*. По окончании работы данного скрипта вы получите новый видеофайл с отрисованными детекциями, а также файл формата *.yml*, с координатами ограничивающих рамок и ключевых точек для каждого кадра.

### Шаг 2: Траектории
Динамическая плотность рассчитывается из траекторий с временными метками. Чтобы их получить, нужно запустить файл [ekf_tracker_test.py](#ekf_tracker_test.py).

### Шаг 3: Динамическая плотность
Для получения результатов подсчета динамической плотности вам нужно запустить сценарий [dynamic_density.py](#dynamic_density.py). Для запуска этого скрипта помимо файла с траекториями нужен файл, хранящий координаты области, для которой нужно произвести подсчет. Для генерации этого файла, запустите файл [mouse_event.py](#mouse_event.py), передав туда путь до исходного видеофайла.
